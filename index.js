  ...
  try {
    ...
    app.put("/api/v1/isBookingSloat/:id", verifyJWT, async (req, res) => {
      ...
      if (isUser.role === "user") {
        ...
        if (req.body.condition === "onsite") {
          const isBookingOnsite = await onsiteBookingCollection.findOne(
            {
              _id: new ObjectId(id),
            },
            {
          ...
          if (isBookingOnsite.isBooked) {
            return res.send({
              status: httpStatus.CONFLICT,
              message: "Already Booking You Can  Not Deleted",
            });
          } else {
            try {
              const result = await onsiteBookingCollection.deleteOne({
                _id: new ObjectId(id),
              });
              return res.send({
                status: httpStatus.OK,
                message: "Successfully OnSite Booking  Delete",
                data: result,
              });
            } catch (error) {
              return res.send({
          ...
          const isBooking = await bookingsCollection.findOne(
            { _id: new ObjectId(id) },
            {
              projection: {
                isBooked: 1,
    ...
    app.delete(`/api/v1/deleteAccount`, verifyJWT, async (req, res) => {
      ...
      try {
        session.startTransaction();
        const result = await usersCollection.deleteOne({ email }, { session });
        if (!result) {
          throw new Error("User Collection Session Failed");
        }
        const patientResult = await patientCollection.deleteOne(
          { email },
          { session }
        );
        if (!patientResult) {
          throw new Error("Patient Collection Session Failed");
        }
    ...
    app.get(
      "/api/v1/doctor_prescription_analysis/:id",
      verifyJWT,
      async (req, res) => {
        const email = req.decoded.email;
        const isDoctor = await usersCollection.findOne(
          {
            email,
          },
          {
        ...
        if ((isDoctor.role = process.env.USER_ROLE)) {
          const id = req.params.id;
          const myprescription = await prescriptionCollection.findOne(
            {
              _id: new ObjectId(id),
            },
            {
          ...
          try {
            const prompt = `Analyze the following prescription details and provide a possible diagnosis or problem the patient is experiencing based on the medicines prescribed:${myprescription.prescription}`;
